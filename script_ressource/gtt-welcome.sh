...
echo "Bienvenue, $USER, dans ce tutoriel pour se familiariser avec Git ! 
...
* modified/untracked : le fichier à été modifié/crée mais n'est pas enregistré dans la base de données.

* staged : le fichier à été modifié et est prêt à être enregistré dans la base de données.

* committed :le fichier est enregistré dans la base de données.
"

shouldloop=true
while $shouldloop
do
	read -p "Entrer y pour passer à la suite " strtotest
	shouldloop=false
